...
MongoDB.prototype.buildWhere = function(modelName, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (cond && cond.constructor.name === 'Object') {
      ...
      console.log(Object.entries(cond));
      const newQuery = Object.entries(cond).reduce((res, [key, value]) => {
        key = trimLeadingDollarSigns(key);
        switch (key) {
          case 'between':
            return {...res, $gte: value[0], $lte: value[1]};
          case 'inq':
            return {
              ...res,
              $in: [].concat(value || []).map(function(x) {
          ...
          case 'nin':
            return {...res,
              $nin: [].concat(value || []).map(function(x) {
                if (isObjectIDProperty(modelCtor, propDef, x, options))
                  return ObjectID(x);
                return x;
              }),
          ...
          case 'like':
            if (value instanceof RegExp) {
              return {...res, $regex: value};
            } else {
              return {...res, $regex: new RegExp(value, regexOptions)};
            }
          case 'nlike':
            if (value instanceof RegExp) {
              return {...res, $not: value};
            } else {
              return {...res, $not: new RegExp(value, regexOptions)};
            }
          case 'neq':
            return {...res, $ne: value};
          case 'regexp':
    ...
    //     query[k] = {};
    //     spec.forEach(operator => {
    //       query[k]['$' + operator] = cond;
    //     });
    //   } else {
    //     query[k] = {};
    //     query[k]['$' + spec] = cond;
